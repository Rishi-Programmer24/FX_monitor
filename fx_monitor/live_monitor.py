...
def fetch_price(
    ...
    Alpha Vantage sometimes returns throttle messages in a 200 response body,
    so the JSON payload needs to be checked explicitly.
    """
    params = {
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if isinstance(data, dict) and ("Note" in data or "Information" in data):
                ...
                print(f"  API message for {pair_name}: {message}")
                time.sleep(60)
                continue
            ...
            print(f"  Network error on {pair_name} (attempt {attempt}/{retries}): {error}")
            time.sleep(2 ** (attempt - 1))
        except (KeyError, TypeError, ValueError) as error:
            print(f"  Couldn't parse response for {pair_name}: {error}")
            return None
